...
main:
  addi $sp, $sp, -256 	# board, ta adicionando o espaço de 256 bits de memoria em sp; 
  li $s1, 1	     				# int gameActive = 1;
  move $s0, $sp         #passa o valor que ta em sp pra s0;
  move $a0, $s0		#passa o valor que ta em s0 pra a0;
  
  jal inicialializeBoard # initializeBoard(board) chama a função e depois volta pra linha de baixo;
  move $a0, $s0 				
  ...
  beqz $s1, end_while
  move $a0, $s0 
  li $a1, 1	#se muudar  pra 1 aparece os locais das bombas
  jal printBoard				 # printBoard(board,0); // Shows the board without bombs
  
  la $a0, msg_row		#ta printando uma mensagem
  ...
  addi $sp, $sp, -4 	#reserva um espaçamento de 4 bits para a coordenada que vai ser reservada
  sw $s0, 0 ($sp)	#guarda o valor de 252 bits
  move $a0, $s2
  move $a1, $s3
  move $a2,$s0
  jal play
